...
- **Prescriptive sources** (`CLAUDE.md`, `TARGET-ARCHITECTURE.md`) define intent — when they're out of date, **flag for Hamid.** Never silently rewrite intent to match a possibly-buggy implementation.
...
7. **Guidelines ↔ behavior** — guidelines don't contradict what the skills actually do (e.g. no skill should be requesting write access silently — check `allowed-tools` and any Bash calls in each SKILL.md for anything beyond read-only API usage).
